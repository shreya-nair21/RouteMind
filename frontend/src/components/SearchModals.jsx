...
export const ActivitySearchModal = ({ isOpen, onClose, onSelect, cityName }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
      <div className="bg-surface rounded-xl w-full max-w-2xl overflow-hidden shadow-lg border border-slate-200 animate-fade-in">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div>
            <h3 className="text-lg font-bold text-secondary uppercase">Explore {cityName}</h3>
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider mt-1">Discover Curated Landmarks</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded bg-surface border border-slate-200 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-all">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {results.map((item, idx) => (
                <div key={idx} className="bg-slate-50 border border-slate-200 p-5 rounded-lg hover:bg-surface hover:border-primary/20 hover:shadow-sm transition-all cursor-pointer group" onClick={() => onSelect({ name: item.name, description: item.description, type: item.type || 'activity', cost: item.cost || 0, startTime: '10:00', duration: '2h' })}>
                   ...
                   <h4 className="font-bold text-secondary uppercase mb-1 tracking-tight text-sm">{item.name}</h4>
                   <p className="text-[11px] text-slate-500 font-normal leading-relaxed line-clamp-2">{item.description}</p>
                </div>
              ))}
            </div>
          )}
...
export const ActivityEditorModal = ({ isOpen, onClose, onSave, activity }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
      <div className="bg-surface rounded-xl w-full max-w-lg shadow-lg border border-slate-200 animate-fade-in overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
           <h3 className="text-base font-bold text-secondary uppercase">Configure Maneuver</h3>
           <button onClick={onClose} className="text-slate-400 hover:text-slate-600"><span className="material-symbols-outlined text-sm">close</span></button>
        </div>
